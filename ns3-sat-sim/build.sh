...
# Configure the build
if [ "$1" == "--debug_all" ]; then
  ./waf configure --build-profile=debug --enable-mpi --enable-examples --enable-tests --enable-gcov --disable-werror --out=build/debug_all || exit 1
  ...
  ./waf configure --build-profile=optimized --enable-mpi --disable-werror --out=build/optimized || exit 1
